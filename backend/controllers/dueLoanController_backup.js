...
const getDueLoansWithoutDateRange = async (req, res) => {
    try {
        ...
        console.log('📅 Fetching due loans for today');
        ...
        // Debug logging
        console.log('🔍 Debug - Initial params:', { page, limit, status, loan_officer_id, branch, performance_class, search, sort_by, sort_order });

        // Status filters for today's due loans
        ...
        // Get total count
        const [countResult] = await sequelize.query(`SELECT COUNT(*) as total FROM loan_schedules ls JOIN loans l ON ls.loan_id = l.id JOIN clients c ON l.client_id = c.id ${whereClause}`, { replacements: summaryReplacements });
        ...
        if (sortField === 'client_name') {
            ...
            orderByClause = `ORDER BY CASE WHEN ls.due_date < CURDATE() THEN DATEDIFF(CURDATE(), ls.due_date) ELSE 0 END ${sortDirection}`;
        } else {
        ...
        const [dueLoans] = await sequelize.query(`SELECT ls.id as schedule_id, ls.installment_number, ls.due_date, ls.principal_due, ls.interest_due, ls.total_due, ls.principal_paid, ls.interest_paid, ls.total_paid, ls.penalty_amount, ls.status as schedule_status, (ls.total_due - ls.total_paid) as outstanding_amount, CASE WHEN ls.due_date < CURDATE() THEN DATEDIFF(CURDATE(), ls.due_date) ELSE 0 END as days_overdue, CASE WHEN ls.due_date < CURDATE() THEN 'Overdue' WHEN ls.due_date = CURDATE() THEN 'Due Today' WHEN ls.due_date <= DATE_ADD(CURDATE(), INTERVAL 7 DAY) THEN 'Due Soon' ELSE 'Future' END as due_status, -- Loan details l.id as loan_id, l.loan_number, l.loan_account, l.applied_amount, l.disbursed_amount, l.loan_balance, l.performance_class, l.branch, l.disbursement_date, -- Client details c.id as client_id, c.client_number, c.first_name as client_first_name, c.last_name as client_last_name, c.mobile as client_mobile, c.email as client_email, CONCAT(c.first_name, ' ', c.last_name) as client_name, -- Loan officer details u.first_name as officer_first_name, u.last_name as officer_last_name, u.employee_id as officer_employee_id, CONCAT(u.first_name, ' ', u.last_name) as officer_name FROM loan_schedules ls JOIN loans l ON ls.loan_id = l.id JOIN clients c ON l.client_id = c.id LEFT JOIN users u ON l.loan_officer_id = u.id ${whereClause} ${orderByClause} LIMIT ? OFFSET ?`, { replacements: mainQueryReplacements });

        // Calculate summary statistics using the same where clause but different replacements
        const [summaryResult] = await sequelize.query(`SELECT COUNT(*) as total_schedules, SUM(ls.total_due - ls.total_paid) as total_outstanding, SUM(CASE WHEN ls.due_date < CURDATE() AND ls.status IN ('pending', 'partial') THEN 1 ELSE 0 END) as overdue_count, SUM(CASE WHEN ls.due_date < CURDATE() AND ls.status IN ('pending', 'partial') THEN (ls.total_due - ls.total_paid) ELSE 0 END) as overdue_amount, SUM(CASE WHEN ls.due_date = CURDATE() AND ls.status IN ('pending', 'partial') THEN 1 ELSE 0 END) as due_today_count, SUM(CASE WHEN ls.due_date = CURDATE() AND ls.status IN ('pending', 'partial') THEN (ls.total_due - ls.total_paid) ELSE 0 END) as due_today_amount, SUM(CASE WHEN ls.due_date BETWEEN DATE_ADD(CURDATE(), INTERVAL 1 DAY) AND DATE_ADD(CURDATE(), INTERVAL 7 DAY) AND ls.status IN ('pending', 'partial') THEN 1 ELSE 0 END) as due_soon_count, SUM(CASE WHEN ls.due_date BETWEEN DATE_ADD(CURDATE(), INTERVAL 1 DAY) AND DATE_ADD(CURDATE(), INTERVAL 7 DAY) AND ls.status IN ('pending', 'partial') THEN (ls.total_due - ls.total_paid) ELSE 0 END) as due_soon_amount FROM loan_schedules ls JOIN loans l ON ls.loan_id = l.id JOIN clients c ON l.client_id = c.id ${whereClause}`, { replacements: summaryReplacements });
...
const getDueLoansWithDateRange = async (req, res) => {
    try {
        ...
        const { start_date, end_date, page = 1, limit = 20, status = 'all', loan_officer_id, branch, performance_class, search, sort_by = 'due_date', sort_order = 'ASC' } = req.query;
        const offset = (page - 1) * limit;
        let whereClause = '';
        let replacements = [];
        let summaryReplacements = [];
